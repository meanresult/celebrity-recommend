# 스크롤 해서 총 60개 게시물 가져올 수 있도록 해야함 
...
def main():
    with sync_playwright() as p:
        ...
        TARGET = 60               # 목표 게시물 개수
        MAX_SCROLL = 7          # 무한 스크롤 방지용(최대 30번까지만)
        SCROLL_Y = 3000           # 한 번 스크롤할 픽셀(시간 아님!)
        WAIT_MS = 5000            # 스크롤 후 로딩 기다리는 시간(밀리초)
        ...
        for s in range(MAX_SCROLL):
            ...
            print(f"[스크롤 {s}] 현재 게시물 링크 수: {count}")

            for idx in range(count):
                link = page.locator("a[href*='/p/']").nth(idx)
            ...
            # 새 게시물 로드될 시간을 줌 (1.5초)
            page.wait_for_timeout(WAIT_MS)

            # 스크롤 후에도 개수가 안 늘면 더 이상 로드가 안 되는 상태일 수 있음
            new_count = page.locator("a[href*='/p/']").count()
            if new_count == count:
                print("📌 스크롤해도 게시물 수가 증가하지 않음 → 종료")
                break           
