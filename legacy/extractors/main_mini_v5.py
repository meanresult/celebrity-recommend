...
def login(page,LOGIN_URL, USERNAME, PW):
    ...
    try:
        #page.wait_for_load_state("button", name="나중에 하기")
...
def extract_post_data(page, href):
    ...
    return insta_id, full_link, src, insta_tag
...
def collect_posts_with_scroll(page, target, max_scroll, scroll_y, wait_ms, KST):
    ...
    for s in range(max_scroll):
        ...
        for idx in range(count):
            ...
            # ✅ 오늘/어제만
            if days_diff == 1:
                insta_id, full_link, src, insta_tag = extract_post_data(page, href)
                posts.append((insta_id, full_link, src, post_date,insta_tag))
...
def save_to_excel(posts, filename="instagram_posts.xlsx"):
    ...
    ws.append(["insta ID", "Post Link", "Image URL", "Post Date", "tag"])

    for insta_id, link, img, post_date, insta_tag in posts:
        ws.append([insta_id, link, img, post_date, insta_tag])
